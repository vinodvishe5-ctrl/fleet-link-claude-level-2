...
| **6** | Tue 21 Jul | **2.F** CLAUDE.md, Skills & Consistency | `day-6/<you>` | **[Full guide →](day-6-consistency.md)** — turn the standards into instruments: grow `CLAUDE.md` + `docs/standards.md`, build a `new-endpoint` skill, a `/standards-check` command, a guardrail **hook** that blocks secrets, and a `standards-reviewer` subagent. Constrain consistency up front. |
| **7** | Wed 22 Jul | **2.G** Multi-Team + **2.H** Debugging & RCA | `day-7/<you>` | **[Full guide →](day-7-multi-team-debugging.md)** — run one role hand-off that adds `GET /api/work-orders/{id}/parts` via the shared `new-endpoint` skill; then reproduce a planted parts-quantity bug from a noisy log, root-cause it, and add an auditable service-layer validation guardrail + regression test. |
